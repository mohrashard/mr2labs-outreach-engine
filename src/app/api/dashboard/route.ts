...
export async function GET() {
  try {
    ...
    const { count: emailsSentToday } = await supabaseAdmin.from('outreach_leads').select('*', { count: 'exact', head: true }).not('sent_at', 'is', null).gte('sent_at', today.toISOString());
    ...
    // Fetch actionable leads (NEW, QUEUED, SENT, REPLIED) without crowding out by REJECTED dead-end rows
